...
Test system convergence across different delta and alpha parameters
for three delay distributions: Spike, Uniform, Bimodal with elasticity fixed.

Variables:
- elasticity: only ε_j0 (col 0, rows 1 & 2) fixed at 16; all other off-diagonals remain 1.5; λ matrix stays 1.5 everywhere
- delta: from 0.001 to 0.125, step 0.001 (tested in reverse order: 0.125 → 0.001)
- alpha_j->0,in: from 1% to 99%, step 1% (tested in reverse order: 99% → 1%)
...
# Elasticity settings
BASE_ELASTICITY = 1.5
FIXED_EPSILON_J0 = 16.0  # epsilon from shard j to shard 0 (rows 1,2 -> col 0)
...
DELTA_POINTS = _points(DELTA_MIN, DELTA_MAX, DELTA_STEP)  # 125 points
ALPHA_POINTS = _points(ALPHA_MIN, ALPHA_MAX, ALPHA_STEP)  # 99 points
TOTAL_POINTS_PER_DISTRIBUTION = DELTA_POINTS * ALPHA_POINTS  # 12375 points per distribution
...
def test_demand_matrix_calculation():
    ...
    # Test with step size matching experiment range: 0.01 from 0.01 to 0.99
    test_alphas = [0.05, 0.25, 0.50, 0.75, 0.95]  # Sample test points
    ...
    print(f"   Step size: {ALPHA_STEP}")
    print(f"   Total points: {ALPHA_POINTS}")
    
    # Test edge cases
    edge_alphas = [ALPHA_MIN, ALPHA_MIN + ALPHA_STEP, ALPHA_MAX - ALPHA_STEP, ALPHA_MAX]
    print(f"\n🧪 Testing edge cases...")
...
def update_config_for_experiment(delta_value, alpha_inflow, delay_weights):
    ...
    try:
        ...
        # Set lambda matrix to base elasticity everywhere
        lambda_size = len(config['demand']['lambda_matrix'])
        lambda_width = len(config['demand']['lambda_matrix'][0])
        config['demand']['lambda_matrix'] = [
            [BASE_ELASTICITY for _ in range(lambda_width)]
            for _ in range(lambda_size)
        ]

        # Set epsilon matrix: diag=0, entries into shard0 fixed to 16, others 1.5
        epsilon_size = len(config['demand']['epsilon_matrix'])
        epsilon_matrix = []
        for i in range(epsilon_size):
            ...
            for j in range(epsilon_size):
                if i == j:
                    ...
                    row.append(FIXED_EPSILON_J0)
                else:
                    row.append(BASE_ELASTICITY)
            epsilon_matrix.append(row)
        config['demand']['epsilon_matrix'] = epsilon_matrix

        # Update delta
        config['simulation']['delta'] = float(delta_value)
        ...
        print(f"✅ Config updated: delta={delta_value:.3f}, alpha={alpha_inflow:.2f}")
        return True
...
def run_single_experiment(delta_value, alpha_inflow, delay_weights, distribution_name, exp_id):
    """Run a single experiment with given parameters"""
    # Update config
    if not update_config_for_experiment(delta_value, alpha_inflow, delay_weights):
        return None
    
    # Create unique log file name for this experiment
    unique_log_name = f"experiment_{distribution_name}_{exp_id:04d}_delta{delta_value:.3f}_alpha{alpha_inflow:.2f}.log"
...
def save_distribution_results(results, distribution_name):
    ...
    filename = f"convergence_results_delta_{distribution_name}.csv"
...
def run_convergence_experiment():
    ...
    print(f"Delta range: {DELTA_MIN} to {DELTA_MAX}, step {DELTA_STEP} ({DELTA_POINTS} points)")
    ...
    try:
        ...
        for dist_name, delay_weights in DELAY_DISTRIBUTIONS.items():
            ...
            # Loop through delta values (从最大值开始倒序)
            for i in range(DELTA_POINTS-1, -1, -1):
                delta_value = round(DELTA_MIN + i * DELTA_STEP, 6)
                ...
                for j in range(ALPHA_POINTS-1, -1, -1):
                    ...
                    print(f"Experiment {experiment_count}/{total_experiments}: " + 
                          f"{dist_name}, δ={delta_value:.3f}, α={alpha_inflow:.2f}")
                    
                    # Run single experiment
                    result = run_single_experiment(delta_value, alpha_inflow, delay_weights, dist_name, experiment_count)
...
def load_and_combine_results():
    ...
    for dist_name in DELAY_DISTRIBUTIONS.keys():
        filename = f"convergence_results_delta_{dist_name}.csv"
...
def create_3d_convergence_plots(df):
    ...
    for i, dist_name in enumerate(distributions):
        ...
        if len(converged) > 0:
            ax.scatter(converged['delta'], converged['alpha_inflow'], 
                      [1] * len(converged), c='green', marker='o', s=20, alpha=0.6, label='Converged')
        
        # Plot diverged points (red) 
        if len(diverged) > 0:
            ax.scatter(diverged['delta'], diverged['alpha_inflow'], 
...
def analyze_convergence_results(df):
    ...
    for dist_name in df['distribution'].unique():
        ...
        if len(converged_subset) > 0:
            max_delta_converged = converged_subset['delta'].max()
            max_alpha_converged = converged_subset['alpha_inflow'].max()
            print(f"   Max converged δ: {max_delta_converged:.3f}")
            print(f"   Max converged α: {max_alpha_converged:.2f}")
    
    # Save summary to file
    summary_file = "convergence_analysis_summary_delta.csv"
    summary_stats = []
...
def visualize_results():
    ...
    if df is not None:
        ...
        df.to_csv("convergence_results_combined_delta.csv", index=False)
        print("💾 Combined results saved to convergence_results_combined_delta.csv")
    else:
