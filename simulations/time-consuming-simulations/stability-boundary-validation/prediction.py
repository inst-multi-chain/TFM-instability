...
def parse_latest_log(delete_after=DELETE_LOG_AFTER_PARSE):
    """
    Parse latest enhanced_simulation_analysis_*.log for 7-shard system
    Supports: step,fee0-6,load0-6 (15 columns total)
    Returns: (dict {0: df0, 1: df1, ..., 6: df6} or None, path_to_log or None)
    After parsing, optionally deletes the log file.
    """
...
def run_single_experiment(epsilon_j0, alpha_inflow, delay_weights, distribution_name, exp_id):
    ...
    if converged and phase_map_stable:
        prediction_category = "True Positive"  # 实际收敛，模型预测收敛（一致）
    elif not converged and not phase_map_stable:
        prediction_category = "True Negative"  # 实际发散，模型预测发散（一致）
    elif not converged and phase_map_stable:
        prediction_category = "False Positive"  # 实际发散，但模型预测收敛（错误）
    else:  # converged and not phase_map_stable
        prediction_category = "False Negative"  # 实际收敛，但模型预测发散（错误）
            ...
            for i in range(EPSILON_POINTS):
                ...
                for j in range(ALPHA_POINTS):
                    ...
                    if experiment_count % 100 == 0:
                        df_dist_temp = pd.DataFrame(distribution_results)
                        df_dist_temp.to_csv(f'phase_map_validation_{dist_name}_temp.csv', index=False)
                        df_all_temp = pd.DataFrame(all_results)
                        df_all_temp.to_csv('phase_map_validation_all_temp.csv', index=False)
                        print(f"💾 Saved intermediate results at experiment {experiment_count}")
...
def analyze_validation_results(df):
    ...
    for dist_name in df['distribution'].unique():
        ...
        print(f"     ✅ True Positive (实际收敛，预测收敛): {true_positive} ({true_positive/len(dist_data)*100:.1f}%)")
        print(f"     ✅ True Negative (实际发散，预测发散): {true_negative} ({true_negative/len(dist_data)*100:.1f}%)")
        print(f"     ❌ False Positive (实际发散，预测收敛): {false_positive} ({false_positive/len(dist_data)*100:.1f}%)")
        print(f"     ❌ False Negative (实际收敛，预测发散): {false_negative} ({false_negative/len(dist_data)*100:.1f}%)")

        # Kappa statistics
        kappa_def_mean = dist_data['kappa_def_95'].mean()
        kappa_att_mean = dist_data['kappa_att_95'].mean()
        print(f"\n   Mean κ_def (95%): {kappa_def_mean:.4f}")
