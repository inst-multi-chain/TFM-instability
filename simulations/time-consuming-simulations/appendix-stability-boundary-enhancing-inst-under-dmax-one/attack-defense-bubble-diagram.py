...
def create_single_bubble_chart(df, distribution_name):
    ...
    # Draw the boundary curve with enhanced visibility and smoothing
    if len(boundary_points) > 1:
        boundary_x, boundary_y = zip(*boundary_points)
        boundary_x, boundary_y = np.array(boundary_x), np.array(boundary_y)
        ...
        ax.plot(boundary_x_smooth, boundary_y_smooth,
    ...
    # Set axis range and format as percentage
    ax.set_xlim(0, 16)
    ...
    if len(diverged_df) > 0:
        ...
        legend_elements.append(mpatches.Patch(
            color='none',
            label=f'Minimum Inst: ε={min_diverged["epsilon_0j"]:.2f}, α={min_diverged["alpha_outflow"]:.2f}'
        ))
    
    ax.legend(handles=legend_elements, fontsize=32, loc='lower left', bbox_to_anchor=(-0.17, 1))
